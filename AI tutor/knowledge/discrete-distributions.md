...
## Page 12 · Game · compute E(X)

Practice: enter E(X)=Σ x f(x) for short discrete distributions, then CHECK. Sample items (with answers): (i) f(0)=0.2, f(1)=0.5, f(2)=0.3 → E(X)=1.1; (ii) f(1)=0.25, f(3)=0.5, f(5)=0.25 → E(X)=3; (iii) Broadway f(0)=0.40,…,f(4)=0.10 → E(X)=1.20; (iv) f(0)=0.1, f(2)=0.6, f(4)=0.3 → E(X)=2.4. Live lecture cycles these with immediate feedback.

## Page 13 · Variance & SD

Var(X)=E[(X−E(X))²]=Σ(x−μ)²f(x)=E(X²)−[E(X)]². σ=√Var(X). Broadway σ²≈1.660, σ≈1.288.

## Page 14 · Variance properties

Var(c)=0. Var(cX)=c² Var(X). For independent X,Y: Var(X+Y)=Var(X)+Var(Y).

## Page 15 · Game · variance quiz

Multiple-choice practice on variance properties. Sample items (with answers): (i) Var(X)=4 → Var(3X+7)=36 (scale by b²; add constant does nothing); (ii) independent Var(X)=2, Var(Y)=5 → Var(X+Y)=7; (iii) computational formula Var(X)=E(X²)−[E(X)]²; (iv) Broadway Var≈1.66 → Var(2X)≈6.64. Live lecture cycles these with feedback.

## Page 16 · Broadway E and Var

Full computation table for Broadway moments (xf(x) and (x−μ)²f(x) columns).

## Page 17 · Binomial properties

(1) n identical trials (2) success/failure each trial (3) constant p (stationarity) (4) independent trials. x = number of successes.

## Page 18 · Binomial formula

f(x)=C(n,x) p^x (1−p)^(n−x). Running story: Café de Coral (大家樂) restaurant chain; try n=3, p=0.10, x=1 → f(1)=0.243.

## Page 19 · Café de Coral

...
/** Poisson PMF: e^(-μ) μ^x / x! */
export function poissonPmf(mu: number, x: number): number {
  if (x < 0 || !Number.isFinite(mu) || mu < 0) return 0;
  return (Math.exp(-mu) * mu ** x) / factorial(x);
}

/** Hypergeometric PMF: C(r,x) C(N-r, n-x) / C(N,n) */
export function hypergeometricPmf(N: number, r: number, n: number, x: number): number {
  if (x < 0 || x > n || x > r || n - x > N - r) return 0;
  const denom = combinations(N, n);
  if (denom === 0) return 0;
  return (combinations(r, x) * combinations(N - r, n - x)) / denom;
}
